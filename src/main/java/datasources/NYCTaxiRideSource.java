...
import throughputUtils.ThroughputStatistics;
 ...
 */
public class NYCTaxiRideSource implements ParallelSourceFunction<Tuple>, ResultTypeQueryable {
    private static transient DateTimeFormatter timeFormatter =
            DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss").withLocale(Locale.US).withZone(ZoneId.of("UTC"));
    ...
    public NYCTaxiRideSource(long runtime, int throughput, final List<Tuple2<Long, Long>> gaps) {
        this.dataFilePath = "/Users/Augustin/Desktop/augustin/uni/Bachelor Thesis/flink-1.14.4/own_projects/firstTest/data/nycTaxiRides.gz";
...
//        this.dataFilePath = "EDADS/Data/nycTaxiRides.gz";
    ...
    public Tuple11<Long, Long, Long, Boolean, Long, Long, Double, Double, Double, Double, Short> fromString(String line) {
        ...
        try {
            ...
            switch (tokens[1]) {
                case "START":
                    ride.f3 = true;
                    ride.f4 = LocalDateTime.parse(tokens[2], timeFormatter).atZone(ZoneId.of("UTC")).toInstant().toEpochMilli();
                    ride.f5 = LocalDateTime.parse(tokens[3], timeFormatter).atZone(ZoneId.of("UTC")).toInstant().toEpochMilli();
                    break;
                case "END":
                    ride.f3 = false;
                    ride.f5 = LocalDateTime.parse(tokens[2], timeFormatter).atZone(ZoneId.of("UTC")).toInstant().toEpochMilli();
                    ride.f4 = LocalDateTime.parse(tokens[3], timeFormatter).atZone(ZoneId.of("UTC")).toInstant().toEpochMilli();
                    break;
                default:
                    throw new RuntimeException("Invalid record: " + line);
            }
    ...
    public TypeInformation getProducedType() {
        return Types.TUPLE(Types.LONG, Types.LONG, Types.LONG, Types.BOOLEAN, Types.LONG, Types.LONG, Types.DOUBLE, Types.DOUBLE, Types.DOUBLE, Types.DOUBLE, Types.SHORT);
    }
}
